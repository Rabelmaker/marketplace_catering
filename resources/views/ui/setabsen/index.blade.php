...
@section('content')
    <div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">Data Setting Absen</h2>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Data Setting Absen</li>
            </ol>
        </div>
        <div class="d-flex">
            <div class="justify-content-center">
                <a href="{{ route('add_setabsen') }}">
                    ...
                    <div class="table-responsive">
                        <table id="exportexample1" class="table table-bordered border-t0 key-buttons text-nowrap w-100">
                            ...
                            <tr>
                                <th>No.</th>
                                <th>Hari</th>
                                <th>Rentang Awal Absen Masuk</th>
                                <th>Rentang Akhir Absen Masuk</th>
                            ...
                            @foreach($datas as $data)
                                <tr>
                                    ...
                                    <td>{{ $data->hari }}</td>
                                    <td>{{ $data->masuk_awal }}</td>
                                    <td>{{ $data->masuk_akhir }}</td>
                                    <td>{{ $data->terlambat }}</td>
                                    <td>{{ $data->pulang }}</td>
                                    <td>
                                        <a href="{{ route('delete_setabsen', $data->id) }}">
                                            <button type="button" class="btn btn-outline-danger my-2 btn-icon-text">
                                                <i class="fe fe-trash-2"></i>
                                            </button>
                                        </a>
                                        <a href="{{ route('edit_setabsen', $data->id) }}">
